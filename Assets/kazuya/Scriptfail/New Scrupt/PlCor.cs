...
public class PlCor : MonoBehaviour
{
    ...
    public bool prri = false;//プレイヤーが動いているかどうかを確認する
    ...
    private bool isGround = false;//プレイヤーが地面についているかの判定
    ...
    public AudioClip SE_chg;//キャラクターチェンジのSE
    public AudioClip SE_jamp;//ジャンプのSE
    ...
    void Start()
    {
        PlayerManager = GameObject.Find("PlayerManager");//プレイヤーオブジェクトの参照
        PlMane = PlayerManager.GetComponent<PlayerManager>();//プレイヤーコンポーネントの取得
        rigidbody = GetComponent<Rigidbody2D>();//リジッドボディの取得
        spriteRenderer = GetComponent<SpriteRenderer>();//スプライトのセット
        audioSource = GetComponent<AudioSource>();//AudioComponentの取得
    ...
    void Update()
    {
        if (PlMane.Alive == true && PlayerNo == 1)
        {
            ...
            if (Input.GetKeyDown(KeyCode.Q) && slope == false)
            {
                ...
                audioSource.PlayOneShot(SE_chg);//チェンジのSEを再生
                index = 0;
            }
    ...
    void Playerchange()
    {
        if (PlayerNo == 1)
        {
            if (index == 1 || index == 4)//四角への変更
            {
                ...
                spriteRenderer.sprite = Square1;//スプライトの更新
                PlMane.Player1.AddComponent<BoxCollider2D>();//コライダーを再コンポーネント
                //変更された値を初期値に戻す
                Squareindex = 0;
                jumpPower = dfjumpPower;
                XSpeed = dfSpeed;//プレイヤーの速度を決定
